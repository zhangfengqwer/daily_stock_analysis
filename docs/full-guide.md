...
| `LONGBRIDGE_OAUTH_CLIENT_ID` | [Longbridge OpenAPI](https://open.longbridge.com/) OAuth client_id；留空且无 Legacy Access Token 时会兼容使用 `LONGBRIDGE_APP_KEY`。**注意：longbridge SDK 不在必需依赖中，需手动 `pip install "longbridge>=4.4.0"`，未安装时该数据源自动跳过** | 可选 |
...
A 股单股资金流默认优先读取同花顺公开单股接口的当日超大单与大单净额，并合成为主力净流入；结果短时缓存以减少重复请求。同花顺不可用时继续回退 AkShare/东方财富。免费同花顺单股接口不提供本链路所需的近 5 日/10 日汇总，因此该备用源下这两个字段保持为空，不会伪造历史累计值。
