...
For A-share single-stock capital flow, the runtime first reads the public THS single-stock endpoint and combines super-large-order and large-order net flow into the current main-force net inflow. Results are cached briefly, and failures still fall back to the existing AkShare/EastMoney path. The free THS endpoint used here does not provide compatible 5-day or 10-day aggregates, so those fields remain empty instead of being inferred.
