...
- [改进] Android 问股页隐藏 AI 回复及加载状态的头像，回复卡片改为占满可用横向空间并收窄消息区内边距，提升手机窄屏下报告和表格的可读宽度；用户头像与桌面 Web 布局保持不变。

- [修复] 移动端问股页排版：Markdown 表格改为容器内横向滚动，避免窄屏下表格列被压到一个字宽；策略选择面板在移动端收进折叠区（折叠标题显示当前选中项），避免 14 个复选框常驻占用近一半竖屏高度。桌面端渲染不变。

- [修复] 认证中间件放行 CORS 预检请求。浏览器按规范发送不带凭证的 `OPTIONS` 预检，原先会被判定未认证并返回 401，该响应不含 CORS 头，导致跨域客户端（移动端 App）的所有需预检请求失败并报出无法定位的 `Failed to fetch`。同源的 Web 与桌面端不发预检，故此前未暴露。实际请求仍照常校验会话。
...
- [改进] 问股新增 `AGENT_STOCK_SCOPE_GUARD_ENABLED` 配置，可按需关闭当前标的工具范围护栏以支持同会话直接追问其他股票代码。
- [修复] 问股实时行情和历史 K 线工具新增市场本地日期元数据，并在对话中注入当前日期约束，避免模型把 6/18 盘中数据误标为 6/19 等未来日期。
